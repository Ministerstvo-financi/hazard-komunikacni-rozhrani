...
if [ -f /lego/certificates/${DOMAIN_NAME}.crt ]; then 
	#renew
	lego --accept-tos --path /lego --webroot /acme --domains "${DOMAIN_NAME}" --email "${ADMIN_EMAIL}" renew --days 15\
	...
	echo first issue
	mkdir /lego
	lego --accept-tos --path /lego --webroot /acme --domains "${DOMAIN_NAME}" --email "${ADMIN_EMAIL}" run \
	&& cp -f /lego/certificates/${DOMAIN_NAME}.crt /etc/apache2/certs/server.crt \
	&& cp -f /lego/certificates/${DOMAIN_NAME}.key /etc/apache2/certs/server.key \
	&& apache2ctl restart
